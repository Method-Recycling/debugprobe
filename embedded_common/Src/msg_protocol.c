...
void msg_protocol_encode(uint8_t value, uint8_t* payload, uint16_t* len) {
	if ((value == PACKET_DELIM_FLAG) || (value == PACKET_ESC_FLAG)) {
		payload[(*len)++] = PACKET_ESC_FLAG;
		payload[(*len)++] = value ^ PACKET_ESC_VAL;
	} else {
		payload[(*len)++] = value;
	}
}
...
void msg_protocol_process(msg_protocol_ctx_t* ctx, uint8_t value) {
	if (!ctx->rx_in_progress) {
		// If a message rx is not in progress and the packet delimiter is received, then start message rx
		if (value == PACKET_DELIM_FLAG) {
			ctx->rx_in_progress = 1;
			ctx->rxd_size = 0;
		}
	} else {
		if (value == PACKET_DELIM_FLAG) {
			// If bytes were received between packet delimiters then process
			if (ctx->rxd_size != 0) {
				msg_protocol_decode(ctx);
				ctx->rx_in_progress = 0;
			}
		} else {
			if (ctx->last_byte_was_esc) {
				ctx->rxd_data[ctx->rxd_size++] = value ^ PACKET_ESC_VAL; // Undo escaped byte
				ctx->last_byte_was_esc = 0;
			} else {
				if (value == PACKET_ESC_FLAG) {
					ctx->last_byte_was_esc = 1; // Set flag so following escaped byte is undone and kept
				} else {
					ctx->rxd_data[ctx->rxd_size++] = value;
				}
			}
		}
	}
